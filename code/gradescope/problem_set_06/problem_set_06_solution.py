...
with open(source_path, "r") as file_obj:
    file_lines_list = file_obj.readlines()
...
def transform_a_list_even(list_of_strings):
    ...
    for index in range(len(list_of_strings)):
        if index%2 == 1:
            new_line = list_of_strings[index]
            for sub_string in sub_strings_to_remove:
                new_line = new_line.replace(sub_string, "")
            result_list.append(new_line)
    
    # Method 2
    result_list = []
    even_lines = list_of_strings[1::2]
...
with open(target_path, "w") as file_obj:
    for index in range(len(transformed_file_lines_list_odd)):
        new_odd_string = transformed_file_lines_list_odd[index].rstrip()
        even_string = transformed_file_lines_list_even[index]
        combined_string = "{}|{}".format(new_odd_string, even_string)
...
def output_words_frequency_from_file(source_file_path, target_file_path, topic_words_list):
    ...
    with open(source_file_path, "r") as file_obj:
        source_file_lines = file_obj.readlines()
    
    with open(target_file_path, "w") as file_obj:
        new_source_file_lines = transform_a_list_odd(source_file_lines)

        for line in new_source_file_lines:
            line_split = line.split()
            for word in line_split:
                if word in topic_words_list:
                    counter = counter + 1
        file_obj.write("{}\n".format(counter))
