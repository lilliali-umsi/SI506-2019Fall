...
for index in love_quotes_indices:
    quote_with_author = love_quotes[index] + " - " + authors[index]
    quotes_with_author.append(quote_with_author)
...
for quote_with_author in quotes_with_author:
    i_wrote_it.append(who_wrote_it(quote_with_author))
i_wrote_it.sort()
...
# Assign the word count values to the variables first_word_count and last_word_count as appropriate. Print first_word_count and last_word_count.
...
def count_words_in_quote(quote):
    if ',' in quote:
        word_num = sum([len(part.split()) for part in quote.split(",")])
    else:
        word_num = len(quote.split())
    return word_num
...
# Use a while loop and is_quote_with_love to identify all quotes without "love", "loving", "loved", "lovable", or "Love".
...
while i < len(love_quotes):
    if not is_quote_with_love(love_quotes[i]):
...
# Reuse functions created above to sum up the word counts of quotes which contain "love", "loving", "loved", or "Love" in love_quotes.
