...
full_name = 'Anthony Whyte' # Your full name
uniqname = 'arwhyte' # Your uniqname
op_sys = op_systems[0] # Your OS by list index position
py_version = '3.8.0' # Your version
tool = tools[0] # Your tool by list index position
...
path = pathlib.Path('lab_exercise_07.py') # FIX ME (add file name)
home_directory = path.home() # use path to return the home directory
current_working_directory = path.cwd() # use path to return the current working directory
file_name = path.name # use path to return the file name
file_name_extension = path.suffix # Use path to return the file name extension (e.g., '.py')
file_path_absolute = path.absolute() # Use path to print out this file's absolute path
