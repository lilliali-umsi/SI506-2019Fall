...
def create_nested_list(fname='planets.txt'):
...
def middle_planets(input=planet_data):
    output = []
    for planet in input:
        if float(planet[4]) > 200.0 and float(planet[4]) < 1500.0:
            output.append(planet[0])
...
# (regardless of lenth) and join them together with a single comma ','. Then, add a new line
...
# For each of the planets, write their names, type, and number of moons to
...
for line in fin:
    if i != 0:
        planet_info = line.split(',')[:3]
        fout.write(string_from_list(planet_info))
    i = i + 1
fout.close()
